							...
							const char character)
{
	...
	{ // lower than space, larger than tilde
		return;
	}
	if (x + 8 - 1 >= VWIDTH || y + 15 - 1 >= VHEIGHT)
	{ // out of bounds
								...
								/*const uint8_t charLimit*/)
{
	uint16_t i = 0;
	const char nullChar[] = "";
	while (string[i] != nullChar[0] && i < 99)
	{
		int x = x0 + 10 * i;
								...
								const char character)
{
	...
	if (x + 16 - 1 >= VWIDTH || y + 30 - 1 >= VHEIGHT)
	{ // out of bounds
		return;
	}
	for (int row = 0; row < 15; row++)
							 ...
							 const float number)
{
	...
	const char numberChar[99] = {0};
	sprintf(numberChar, "%.4f", number);
								 ...
								 const float number)
{
	...
	drawString2x(bitmap, x0 + offset, y0, color, numberChar, charLimit);
}
							 ...
							 const int number)
{
	...
	drawString2x(bitmap, x0 + offset, y0, color, numberChar, 50);
}
