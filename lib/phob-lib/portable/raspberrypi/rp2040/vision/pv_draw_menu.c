									...
									const StickParams_s *cStick)
{
	...
	case 0:
		...
		if (controls->rConfig == 4)
		{
			drawString(bitmap, 30, 150, 15, l5conflict);
		}
		break;
	case 1:
	...
	case 2:
		...
		if (controls->rConfig == 4)
		{
			drawString(bitmap, 30, 170, 15, l5conflict);
		}
		break;
	case 3:
		...
		if (controls->lTriggerOffset < 80)
		{
			drawString(bitmap, 30, 190, 15, lrultimate);
		}
		if (controls->rConfig == 4)
		{
			drawString(bitmap, 30, 210, 15, l5conflict);
		}
		break;
	case 4:
		...
		if (controls->lTriggerOffset < 80)
		{
			drawString(bitmap, 30, 210, 15, lrultimate);
		}
		if (controls->rConfig != 1 && controls->rConfig != 4 && controls->rConfig != 5)
		{
			drawString(bitmap, 30, 230, 15, trigger5);
									...
									const StickParams_s *cStick)
{
	...
	case 0:
		...
		if (controls->lConfig == 4)
		{
			drawString(bitmap, 30, 150, 15, r5conflict);
		}
		break;
	case 1:
		drawString(bitmap, 30, 130, 15, lrtrigger5);
	...
	case 2:
		...
		if (controls->lConfig == 4)
		{
			drawString(bitmap, 30, 170, 15, r5conflict);
		}
		break;
	case 3:
		drawString(bitmap, 30, 130, 15, lrtrigger9);
		...
		drawString(bitmap, 30, 150, 15, lrtrigger10);
		drawString(bitmap, 30, 170, 15, lrtrigger11);
		if (controls->rTriggerOffset < 80)
		{
			drawString(bitmap, 30, 190, 15, lrultimate);
		}
		if (controls->lConfig == 4)
		{
			drawString(bitmap, 30, 210, 15, r5conflict);
		}
		break;
	case 4:
		drawString(bitmap, 30, 130, 15, lrtrigger12);
								 ...
								 DataCapture_s *capture)
{
	...
	if (itemIndex == 1)
	{
		if (capture->captureStick == C_STICK)
		{
	...
	if (capture->captureStick == A_STICK)
	{
										 ...
										 DataCapture_s *capture)
{
	...
	if (capture->abxyszrl[0] & 0b0000'0001)
	{ // A
		a = -1;
	}
	if (capture->abxyszrl[0] & 0b0000'0010)
	{ // B
		b = -1;
	}
	if (capture->abxyszrl[0] & 0b0000'0100)
	{ // X
		x = -1;
	}
	if (capture->abxyszrl[0] & 0b0000'1000)
	{ // Y
		y = -1;
	}
	if (capture->abxyszrl[0] & 0b1000'0000)
	{ // L
		l = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0010'0000)
	{ // La
		la = -1;
	}
	if (capture->abxyszrl[0] & 0b0100'0000)
	{ // R
		r = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0001'0000)
	{ // Ra
		ra = -1;
	}
	if (capture->abxyszrl[0] & 0b0010'0000)
	{ // Z
		z = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0000'0001)
	{ // Ax
		ax = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0000'0010)
	{ // Ay
		ay = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0000'0100)
	{ // Cx
		cx = -1;
	}
	if (capture->axaycxcyrl[0] & 0b0000'1000)
	{ // Cy
		cy = -1;
	}
									...
									const StickParams_s *cStick)
{
	...
	{
	case MENU_ASTICKCAL:
		drawStickCalFast(bitmap, menu, currentCalStep, changeMade, A_STICK, btn, raw, controls, aStick, cStick);
		break;
	case MENU_CSTICKCAL:
		drawStickCalFast(bitmap, menu, currentCalStep, changeMade, C_STICK, btn, raw, controls, aStick, cStick);
		break;
	case MENU_STICKDBG:
							...
							DataCapture_s *capture)
{
	...
	{
	case MENU_ASTICKCAL:
		drawStickCal(bitmap, menu, currentCalStep, changeMade, A_STICK, btn, raw, controls, aStick, cStick);
		break;
	case MENU_CSTICKCAL:
		drawStickCal(bitmap, menu, currentCalStep, changeMade, C_STICK, btn, raw, controls, aStick, cStick);
		break;
	case MENU_AUTOINIT:
