																						 ...
																						 DataCapture_s *capture)
{
	...
	{
		backAccumulator++;
		if (backAccumulator == 5)
		{ // only call save once per 0.5 second B hold
									...
									DataCapture_s *capture)
{
	...
	else if (MenuIndex[*menu][1] > 0)
	{
		...
		case MENU_TIMESCOPE: // value vs time plot
			...
			else if (presses * DDPRESS)
			{
				if (itemIndex == 0)
				{
					...
					case 0:
						capture->captureStick = ASTICK;
						capture->whichAxis = XAXIS;
						capture->mode = (tempInt2 == 0) ? CM_STICK_FALL : (tempInt2 == 1) ? CM_STICK_RISE
																																							: CM_STICK_PIVOT;
						break;
					case 1:
						capture->captureStick = ASTICK;
				...
				else if (itemIndex == 1)
				{
					...
					{ // only applies when the sticks are selected
						tempInt2--;
						capture->mode = (tempInt2 == 0) ? CM_STICK_FALL : (tempInt2 == 1) ? CM_STICK_RISE
																																							: CM_STICK_PIVOT;
					}
			...
			{ // this will be triggered by autorepeat
